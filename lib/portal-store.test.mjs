...
import { getTeacherState, getStudentState, submitApplication, verifyApplication, importStudents, updateClubs, changePhase, runAllocation, runClubAllocation, updateFirstRankExclusions, adjustPlacement, finalize, getHistory, getAudit, resetStudentCode } from './server/store.ts';
...
test('first preference exclusion settings are private, audited, revision guarded and locked after allocation without touching applications',async context=>{
 const {db}=await fixture(context,3);
 await submitApplication(db,'5-1-1',preferences,0);
 await submitApplication(db,'5-1-2',preferences,0);
 let state=await getTeacherState(db);
 state=await changePhase(db,'closed',state.revision,actor);
 const before=state,history=await getHistory(db,'5-1-1');
 for(const ids of [['missing'],['5-1-1','5-1-1'],[null]]) await assert.rejects(updateFirstRankExclusions(db,'paper',ids,state.revision,actor));
 state=await updateFirstRankExclusions(db,'paper',['5-1-1','5-1-3'],state.revision,actor);
 assert.equal(state.phase,'closed');assert.equal(state.result,null);
 assert.deepEqual(state.students,before.students);
 assert.deepEqual(await getHistory(db,'5-1-1'),history);
 const own=await getStudentState(db,'5-1-1');
 assert.ok(!JSON.stringify(own).includes('firstRankExcludedStudentIds'));
 const audit=(await getAudit(db)).at(-1);assert.equal(audit.action,'clubs.first-rank-exclusions');
 assert.deepEqual(audit.payload.after,['5-1-1','5-1-3']);
 await assert.rejects(updateFirstRankExclusions(db,'paper',[],before.revision,actor),isConflict);
 await assert.rejects(updateClubs(db,state.clubs.map(c=>c.id==='paper'?{...c,firstRankExcludedStudentIds:[]}:c),state.revision,actor));
 await assert.rejects(updateClubs(db,state.clubs.map(c=>c.id==='paper'?{...c,allocationMode:'fixed',fixedStudentIds:['5-1-1']}:c),state.revision,actor));
 state=await runAllocation(db,'private-exclusions',state.revision,actor);
 assert.equal(state.result.placements['5-1-1'].club,null);
 assert.equal(state.result.placements['5-1-2'].club,'paper');
 const afterFirst=state;
 await assert.rejects(updateFirstRankExclusions(db,'paper',[],state.revision,actor),isConflict);
 assert.deepEqual(await getTeacherState(db),afterFirst);
 state=await runClubAllocation(db,'pen',2,1,state.revision,actor);
 assert.equal(state.result.placements['5-1-1'].club,'pen');
 assert.equal(state.result.placements['5-1-1'].rank,2);
 assert.deepEqual(state.students,before.students);
});
