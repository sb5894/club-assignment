...
test('teacher API requires the expected preference round and allows only fixed roster edits after closing',async context=>{
  const {env,teacher,codes}=await classroom(context);
  const student=await studentLogin(env,codes[0]);
  const submission=await request(env,'/api/student/application',{method:'POST',cookie:student,body:{choices:['paper','pen','writing'],version:0}});
  assert.equal(submission.status,200);
  let state=await teacherState(env,teacher);
  ({state}=await teacherAction(env,teacher,{action:'phase',phase:'closed',revision:state.revision}));
  for(const body of [
    {action:'allocate',seed:'api-stages',revision:state.revision},
    {action:'allocate',seed:'api-stages',rank:2,revision:state.revision},
    {action:'settings',clubs:state.clubs.map(c=>({...c,max:c.max+1})),revision:state.revision},
  ]) {
    const response=await request(env,'/api/teacher/action',{method:'POST',cookie:teacher,body});
    assert.ok([400,409].includes(response.status));
    assert.deepEqual(await teacherState(env,teacher),state);
  }
  const settings={action:'settings',clubs:state.clubs.map(c=>c.id==='pen'?{...c,allocationMode:'fixed',fixedStudentIds:[codes[1].id]}:c),revision:state.revision};
  const denied=await request(env,'/api/teacher/action',{method:'POST',cookie:student,body:settings});
  assert.ok([401,403].includes(denied.status));
  ({state}=await teacherAction(env,teacher,settings));
  const exclusion={action:'first-rank-exclusions',clubId:'paper',studentIds:[codes[2].id],revision:state.revision};
  const exclusionDenied=await request(env,'/api/teacher/action',{method:'POST',cookie:student,body:exclusion});
  assert.ok([401,403].includes(exclusionDenied.status));
  ({state}=await teacherAction(env,teacher,exclusion));
  const sanitized=await request(env,'/api/student/me',{cookie:student});
  assert.ok(!(await sanitized.text()).includes('firstRankExcludedStudentIds'));

  for(let rank=1;rank<=3;rank++) {
    if(rank===1) ({state}=await teacherAction(env,teacher,{action:'allocate',seed:'api-stages',rank,revision:state.revision}));
    else {
      const bulk=await request(env,'/api/teacher/action',{method:'POST',cookie:teacher,body:{action:'allocate',seed:'api-stages',rank,revision:state.revision}});
      assert.equal(bulk.status,409,'old bulk APIs cannot bypass per-club quotas');
      const forbidden=await request(env,'/api/teacher/action',{method:'POST',cookie:student,body:{action:'allocate-club',clubId:'paper',rank,seats:0,revision:state.revision}});
      assert.ok([401,403].includes(forbidden.status));
      for(const club of state.clubs) ({state}=await teacherAction(env,teacher,{action:'allocate-club',clubId:club.id,rank,seats:0,revision:state.revision}));
    }
    assert.equal(state.result.completedRank,rank);
    assert.equal(state.result.placements[codes[0].id].rank,1,'unverified submitted preferences remain eligible');
    const own=await request(env,'/api/student/me',{cookie:student});
    assert.equal((await own.json()).placement,null);
    if(rank<3) {
      const premature=await request(env,'/api/teacher/action',{method:'POST',cookie:teacher,body:{action:'finalize',revision:state.revision}});
      assert.equal(premature.status,409);
    }
  }
  ({state}=await teacherAction(env,teacher,{action:'finalize',revision:state.revision}));
