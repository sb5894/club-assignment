...
import { createPasswordHash, hashSecret, normalizeStudentCode } from './server/auth.ts';
...
test('student codes and session tokens are stored as hashes and never appear in persistent audit payloads', async context => {
  ...
  for (const secret of [password, rawToken, teacher.split('=')[1], ...codes.flatMap(code => [code.code, normalizeStudentCode(code.code)])]) assert.ok(!persistent.includes(secret));
