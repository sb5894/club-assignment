...
test('first preference exclusions affect only that club at rank one and preserve later preference eligibility',()=>{
 const applicants=students.slice(0,3).map((s,i)=>({...s,choices:i===2?['pen','paper','writing']:['paper','pen','writing']}));
 const clubs=CLUBS.map(c=>({...c,min:0,max:c.id==='pen'?1:5,firstRankExcludedStudentIds:c.id==='paper'?[applicants[0].id,applicants[2].id]:[]}));
 const first=allocateNext(applicants,clubs,'exclusions');
 assert.equal(first.placements[applicants[0].id].club,null);
 assert.equal(first.placements[applicants[1].id].club,'paper');
 assert.equal(first.placements[applicants[2].id].club,'pen');
 const log=first.rounds.find(r=>r.club==='paper');
 assert.deepEqual(log.excluded,[applicants[0].id]);
 assert.ok(!log.candidates.includes(applicants[0].id));
 let next=allocateNext(applicants,clubs,first.seed,first);
 assert.ok(next.rounds.find(r=>r.rank===2&&r.club==='pen').candidates.includes(applicants[0].id));
 next=allocateNext(applicants,clubs,first.seed,next);
 assert.equal(next.placements[applicants[0].id].club,'writing');
 assert.equal(next.placements[applicants[0].id].rank,3);
 assert.deepEqual(applicants[0].choices,['paper','pen','writing']);
 const conflict=clubs.map(c=>c.id==='paper'?{...c,allocationMode:'fixed',fixedStudentIds:[applicants[0].id]}:c);
 assert.throws(()=>allocateNext(applicants,conflict,'conflict'),/동시에 지정/);
 const secondChoice=allocate(applicants,clubs.map(c=>({...c,max:c.id==='pen'?1:5,firstRankExcludedStudentIds:c.id==='pen'?[applicants[0].id,applicants[2].id]:c.firstRankExcludedStudentIds})),'second-eligible');
 assert.equal(secondChoice.placements[applicants[0].id].club,'pen');
 assert.equal(secondChoice.placements[applicants[0].id].rank,2);
});
