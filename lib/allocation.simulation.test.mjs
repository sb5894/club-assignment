...
function inspect(students, clubs, result) {
  ...
  for (const club of clubs) {
    ...
    const actual = Object.entries(result.placements).filter(([, placement]) => placement.club === club.id && placement.assignmentType === 'fixed').map(([id]) => id).sort(compareIds);
  ...
  const lotteryClubs = [...clubs].sort((a, b) => a.id.localeCompare(b.id));
  ...
  for (const student of students) {
    ...
    for (const id of skippedChoices) {
      const club = clubs.find(club => club.id === id);
      assert.equal(occupancy[id], club.max, `${student.id}: skipped preference has no remaining seats`);
    }
  }
...
for (let index = 0; index < 10; index++) {
  audit(`simulation ${index + 1}: 250 students, exact rosters, lottery eligibility, quotas and replay`, () => {
    ...
    for (const club of clubs.filter(isFixed)) {
      const idsFor = draw => Object.entries(draw.placements).filter(([, placement]) => placement.club === club.id && placement.assignmentType === 'fixed').map(([id]) => id).sort(compareIds);
...
audit('empty fixed roster participates in the ordinary lottery with its full capacity', () => {
  const { students, clubs } = fixture();
  ...
  assert.equal(result.rounds.filter(round => round.club === 'paper').length, 3);
  assert.equal(result.rounds.filter(round => round.club === 'counsel').length, 3);
  assert.equal(result.rounds.find(round => round.club === 'paper' && round.rank === 1).seats, 25);
  for (const student of students.slice(2)) assert.deepEqual(result.placements[student.id], { club: 'paper', rank: 1 });
});

audit('manual moves protect fixed students and allow others into and out of partially fixed clubs', () => {
  const { students, clubs } = fixture();
  ...
  const entered = moveStudent(result, students, clubs, students[2].id, 'counsel', 'use remaining seat');
  assert.equal(entered.placements[students[2].id].club, 'counsel');
  assert.equal(entered.placements[students[2].id].assignmentType, 'manual');
  const moved = moveStudent(entered, students, clubs, students[2].id, 'pen', '  student reselection  ');
  ...
  for (const student of students.slice(0, 2)) assert.deepEqual(moved.placements[student.id], result.placements[student.id]);
...
audit('a 25-seat club with 5 fixed students admits exactly 20 first-choice lottery winners', () => {
  const students = demoStudents(35, 'five-fixed-twenty-lottery').map(student => ({ ...student, choices: ['paper', 'pen', 'writing'] }));
  const clubs = copyClubs();
  Object.assign(clubs.find(club => club.id === 'paper'), { allocationMode: 'fixed', fixedStudentIds: students.slice(0, 5).map(student => student.id) });
  const result = allocate(students, clubs, 'five-fixed-twenty-lottery');
  const round = result.rounds.find(round => round.club === 'paper' && round.rank === 1);
  assert.equal(round.seats, 20);
  assert.equal(round.candidates.length, 30);
  assert.equal(round.winners.length, 20);
  const paper = Object.values(result.placements).filter(placement => placement.club === 'paper');
  assert.equal(paper.length, 25);
  assert.equal(paper.filter(placement => placement.assignmentType === 'fixed').length, 5);
  assert.equal(paper.filter(placement => placement.rank === 1).length, 20);
});

audit('remaining seats fill in rank 1, then rank 2, then rank 3 while fixed students stay assigned', () => {
  const students = demoStudents(8, 'fixed-with-three-rounds').map(student => ({ ...student, choices: ['paper', 'pen', 'writing'] }));
  students[5].choices = ['pen', 'paper', 'writing'];
  students[6].choices = ['pen', 'writing', 'paper'];
  students[7].choices = ['pen', 'writing', 'paper'];
  const clubs = copyClubs();
  Object.assign(clubs.find(club => club.id === 'paper'), { min: 0, max: 5, allocationMode: 'fixed', fixedStudentIds: students.slice(0, 2).map(student => student.id) });
  Object.assign(clubs.find(club => club.id === 'pen'), { min: 0, max: 1, allocationMode: 'fixed', fixedStudentIds: [students[2].id] });
  Object.assign(clubs.find(club => club.id === 'writing'), { min: 0, max: 1, allocationMode: 'fixed', fixedStudentIds: [students[3].id] });
  const result = allocate(students, clubs, 'fixed-with-three-rounds');
  const rounds = result.rounds.filter(round => round.club === 'paper');
  assert.deepEqual(rounds.map(round => round.seats), [3, 2, 1]);
  assert.deepEqual(rounds.map(round => round.winners.length), [1, 1, 1]);
  assert.deepEqual(rounds.map(round => round.candidates.length), [1, 1, 2]);
  assert.equal(result.placements[students[4].id].rank, 1);
  assert.equal(result.placements[students[5].id].rank, 2);
  assert.equal(students.slice(6).filter(student => result.placements[student.id].rank === 3).length, 1);
  assert.equal(students.slice(6).filter(student => result.placements[student.id].club === null).length, 1);
  for (const student of students.slice(0, 4)) {
    assert.equal(result.placements[student.id].assignmentType, 'fixed');
    assert.equal(result.placements[student.id].rank, 0);
  }
});

audit('a full fixed roster leaves zero lottery seats and rejects extra manual assignments', () => {
  const { students, clubs } = fixture();
  Object.assign(clubs.find(club => club.id === 'counsel'), { min: 0, max: 2 });
  for (const student of students.slice(2)) student.choices = ['counsel', 'paper', 'pen'];
  const result = allocate(students, clubs, 'full-fixed-roster');
  for (const round of result.rounds.filter(round => round.club === 'counsel')) {
    assert.equal(round.seats, 0);
    assert.deepEqual(round.winners, []);
  }
  assert.equal(Object.values(result.placements).filter(placement => placement.club === 'counsel').length, 2);
  for (const student of students.slice(2)) assert.equal(result.placements[student.id].rank, 2);
  assert.throws(() => moveStudent(result, students, clubs, students[2].id, 'counsel', 'no vacancy'));
});

audit('fixed students are excluded from every candidate list even when their preferences name other clubs', () => {
  const { students, clubs } = fixture();
  const result = allocate(students, clubs, 'fixed-other-preferences');
  for (const student of students.slice(0, 2)) {
    assert.ok(!student.choices.includes('counsel'));
    assert.equal(result.placements[student.id].club, 'counsel');
    assert.equal(result.placements[student.id].assignmentType, 'fixed');
    for (const round of result.rounds) {
      assert.ok(!round.candidates.includes(student.id));
      assert.ok(!round.winners.includes(student.id));
    }
  }
  const winners = result.rounds.flatMap(round => round.winners);
  assert.equal(winners.length, new Set(winners).size);
  assert.equal(winners.length + 2, students.length);
});
...
after(async () => {
  const report = {
    ...
    passed: checks.length === 20 && checks.every(check => check.passed) && simulations.length === 10,
